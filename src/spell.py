...
class Spell:
    """Represents a Dungeons & Dragons spell with key attributes and
    validation.

    Attributes:
        MAX_TITLE_CHARS (int): Max character length for the spell title.
        MAX_DESCRIPTION_CHARS (int): Max character length for the description.
        ...
        description (str): Description of the spell's effect and mechanics.
        school (str): Magical school the spell belongs to (e.g., Evocation).
        level (int): Spell level, must be between 0 (cantrip) and 9 (highest).
    """
    ...
    def __init__(
        ...
        """Initialize a Spell object with validation checks on title length,
        component format, and level.

        Raises:
            ValueError: If title exceeds MAX_TITLE_CHARS.
            ValueError: If components are not one or more of 'V', 'S', or 'M',
                        comma-separated and unique.
            ValueError: If level is not between 0 and 9.
        """
        if len(title) > Spell.MAX_TITLE_CHARS:
            raise ValueError(
                f"Title {title} is too long. "
        ...
        if not self.is_valid_components(components):
            raise ValueError(
                ...
                "separated by comma and not repeated."
            )
    ...
    def is_valid_components(s: str) -> bool:
        """Validates that the components string contains only V, S, and M,
        comma-separated, and not repeated.
